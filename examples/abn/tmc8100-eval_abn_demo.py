...
win = tk.Tk()
win.title("TMC8100-EVAL ABN-Encoder Demo")
...
def spi_abn_encoder_value():
    # get encoder value
    value = SpiWriteCommand([0x80, 0x00, 0x00, 0x00])
    WaitForGPIO6()
    # get value
    value = SpiWriteCommand([0x00, 0x00, 0x00, 0x00])
    print(f"Encoder Position: {value[0]:02x} {value[1]:02x} {value[2]:02x} {value[3]:02x}")
    absolute = value[3] + 256*value[2] + 65536*value[1]
    encoder_value_text.set(absolute)
   
def spi_abn_encoder_version():
...
## ABN Encoder
##
fm_abn = tk.LabelFrame(text="Encoder Position Counter")
btn_abn_counter = tk.Button(master=fm_abn, text="Counter", command=spi_abn_encoder_value); btn_abn_counter.grid(row = 0,column = 0)
lbl_encoder_value = tk.Label(master=fm_abn, text="Counter Value"); lbl_encoder_value.grid(row=0, column=1)
encoder_value_text = tk.StringVar()
ent_encoder_value = tk.Entry(master=fm_abn, textvariable=encoder_value_text, width=12); ent_encoder_value.grid(row=0, column=2)
encoder_value_text.set("0")

fm_firmware_version = tk.LabelFrame(text="FW Version")
btn_get_firmware_version = tk.Button(master=fm_firmware_version, text="FW Version", command=spi_abn_encoder_version); btn_get_firmware_version.grid(row = 0,column = 0)
btn_get_protocol = tk.Button(master=fm_firmware_version, text="Get Protocol", command=spi_abn_encoder_protocol); btn_get_protocol.grid(row = 1,column = 0)
...
fm_abn.grid(sticky="W", row=2, column=0)
