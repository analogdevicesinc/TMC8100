...
win = tk.Tk()
win.title("TMC8100-EVAL EnDAT 2.x Demo")
...
def spi_get_encoder_value_cdf0():
    ...
    encoder_flags_text.set(value[1])
...
## EnDAT 2.2
##
fm_endat = tk.LabelFrame(text="EnDAT 2.2")
btn_get_encoder_value_endat = tk.Button(master=fm_endat, text="Get Encoder position value", command=spi_get_encoder_value_cdf0); btn_get_encoder_value_endat.grid(row = 0,column = 0)
btn_get_encoder_value_cont = tk.Button(master=fm_endat, text="Read Encoder continuously", command=spi_get_encoder_value_cont); btn_get_encoder_value_cont.grid(row = 1,column = 0)
lbl_encoder_single_turn = tk.Label(master=fm_endat, text="ST"); lbl_encoder_single_turn.grid(row=0, column=1)
encoder_single_turn_text = tk.StringVar()
ent_encoder_single_turn = tk.Entry(master=fm_endat, textvariable=encoder_single_turn_text, width=7); ent_encoder_single_turn.grid(row=0, column=2)
encoder_single_turn_text.set("0")
lbl_encoder_multiturn = tk.Label(master=fm_endat, text="MT"); lbl_encoder_multiturn.grid(row=0, column=3)
encoder_multiturn_text = tk.StringVar()
ent_encoder_multiturn = tk.Entry(master=fm_endat, textvariable=encoder_multiturn_text, width=6); ent_encoder_multiturn.grid(row=0, column=4)
encoder_multiturn_text.set("0")
lbl_encoder_flags = tk.Label(master=fm_endat, text="Flags"); lbl_encoder_flags.grid(row=0, column=5)
encoder_flags_text = tk.StringVar()
ent_encoder_flags = tk.Entry(master=fm_endat, textvariable=encoder_flags_text, width=6); ent_encoder_flags.grid(row=0, column=6)
encoder_flags_text.set("0")
lbl_crc_received = tk.Label(master=fm_endat, text="CRC received"); lbl_crc_received.grid(row=0, column=7)
encoder_crc_received_text = tk.StringVar()
ent_crc_received = tk.Entry(master=fm_endat, textvariable=encoder_crc_received_text, width=6); ent_crc_received.grid(row=0, column=8)
encoder_crc_received_text.set("0")
lbl_crc_calculated = tk.Label(master=fm_endat, text="CRC calculated"); lbl_crc_calculated.grid(row=0, column=9)
encoder_crc_calculated_text = tk.StringVar()
ent_crc_calc = tk.Entry(master=fm_endat, textvariable=encoder_crc_calculated_text, width=6); ent_crc_calc.grid(row=0, column=10)
...
fm_endat.grid(sticky="W", row=2, column=0)
...
def read_every_second():
    ...
    if reply == 1: # reply in SPI transmit buffer
        ...
        single_turn = 65536 * value[1] + 256 * value[2] + value[3]
        encoder_single_turn_text.set(single_turn)
        # get MT value
        value = SpiWriteCommand([0x00, 0x00, 0x00, 0x00])
        multi_turn = value[2] * 256 + value[3]
        ...
        # use single turn 23bit value
        absolute_degree = single_turn / 8388608 * 360
