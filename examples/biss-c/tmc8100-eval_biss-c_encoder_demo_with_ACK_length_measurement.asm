...
;
	...
	BISS_FREQ_DIV = 100 - 1 ;  
	; BISS_FREQ_DIV = 99 ;  1MHz BiSS clock -> 100Mhz / 100
	...
	; BISS_TOGGLE_DIV = 49 ; 1MHz BiSS clock ->  2MHz toggle rate -> 100MHz / 50
	; BISS_TOGGLE_DIV = 24 ;  2MHz BiSS clock ->  4MHz toggle rate -> 100MHz / 25
	; BISS_TOGGLE_DIV = 9  ;  5MHz BiSS clock -> 10MHz toggle rate -> 100MHz / 10
	; BISS_TOGGLE_DIV = 4  ; 10MHz BiSS clock -> 20MHz toggle rate -> 100MHz / 5
	...
	BISS_CLK_EDGES = 54 ;  minimum / without processing delay
	; BISS_CLK_EDGES = 54 ;  1MHz BiSS clock -> rising and falling edges
	; BISS_CLK_EDGES = 56 ;  2MHz BiSS clock -> rising and falling edges
	; BISS_CLK_EDGES = 62 ;  5MHz BiSS clock -> rising and falling edges
	; BISS_CLK_EDGES = 74 ; 10MHz BiSS clock -> rising and falling edges
...
WAIT_FOR_PLL:
	...
	LDI 0, r7 ; will contain number of clock edges after initialization
...
BISS_encoder:
	...
	; for the first encoder read access r7 equals zero and a dummy encoder read
	; is performed to measure the length (number of bits) of the ACK signal 
	; - dependent on clock frequency
	; r7 = BISS_CLK_EDGES + 2x additional ACK bits
	LDI 0, r0
	COMP NE r0, r7
	JC INIT_DONE
	; dummy read for measuring encoder processing time
    ; init counter for BiSS-C clock generation
	...
	LDI BISS_CLK_EDGES,r0 ; this is without additional clock cycles for ACK bits > 1
	...
	LDI BISS_FREQ_DIV, r0
	WAIT0SF WAIT_IN1, WAIT_START_TIMER ; wait for falling edge of data signal '0'
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; wait for middle of (first) ACK bit / half a clock sycle
	STS r0, SYSTEM_TIMER, SYSTEM_TIMER_LIMIT_NO_RESET_W ; next sample point -> full clock cycle
SEARCH_FOR_START:	
	SHLI WAIT1SF WAIT_OVERFLOW_TIMER, r0, FLAG_IN1 ; shift in SL data signal at sample point
	TEST1 0, r0 ; middle of START bit - SL = '1' ? 
	JC START_FOUND
	INC r7, r7
	JA SEARCH_FOR_START
START_FOUND:
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; middle of CDS bit
	REP 5, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; upper 5bit of encoder data
	REP 8, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; lower 8bit of encoder data
	REP 2, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; error and warning
	REP 6, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; crc checksum
	; add timeout before starting the "real" encoder read access
	LDI 199, r0
	STS r0, SYSTEM_TIMER, SYSTEM_TIMER_LIMIT_W
	REP 8, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; 8x 2us = 16us
	REP 8, 1
	WAIT1SF WAIT_OVERFLOW_TIMER, WAIT_NO_ACTION ; 8x 2us = 16us
	; disable (and reset) counter & timer
    LDI 0, r0 ; disable counter & timer
    STS r0, SYSTEM_TIMER, SYSTEM_TIMER_CTRL_W
	; initialization done - r7 contains number of ACK bits / encoder processing delay
	; double number to get number of edges
	ADD r7, r7, r7
	; add number of edges for the whole datagram (independent from clock frequency)
...
INIT_DONE:
	...
	STS r0, SYSTEM_TIMER, SYSTEM_TIMER_COUNTER_LIMIT_W
	; number of rising and falling edges
	STS r7, SYSTEM_TIMER, SYSTEM_TIMER_PULS_COUNTER_LIMIT_W
    LDI 1, r0 ; enable counter
    STS r0, SYSTEM_TIMER, SYSTEM_TIMER_CTRL_W
    ; HEADER
	; '0'
	...
	LDI BISS_FREQ_DIV, r0 ; set sample point to middle of next bit (CDS) / 1 clock cycle
	...
	; we are done
	; note: there is still the bus timeout / next encoder read request must wait until end of timeout
    ; ****************************************
