...
win = tk.Tk()
win.title("TMC8100-EVAL BiSS-C Demo")
...
def spi_get_encoder_value():
    ...
    encoder_clock_edges_text.set(value[3])
...
## BiSS-C fm_biss
##
fm_biss = tk.LabelFrame(text="BiSS-C")
btn_get_encoder_value = tk.Button(master=fm_biss, text="Read Encoder", command=spi_get_encoder_value); btn_get_encoder_value.grid(row = 0,column = 0)
btn_get_encoder_value_cont = tk.Button(master=fm_biss, text="Read Encoder continuously", command=spi_get_encoder_value_cont); btn_get_encoder_value_cont.grid(row = 1,column = 0)

lbl_encoder_single_turn = tk.Label(master=fm_biss, text="ST (raw / gray code / 13bit encoder value)"); lbl_encoder_single_turn.grid(row=0, column=1)
encoder_single_turn_text = tk.StringVar()
ent_encoder_single_turn = tk.Entry(master=fm_biss, textvariable=encoder_single_turn_text, width=7); ent_encoder_single_turn.grid(row=0, column=2)
encoder_single_turn_text.set("0")

lbl_encoder_single_turn_converted = tk.Label(master=fm_biss, text="ST (converted / binary / 13bit encoder value)"); lbl_encoder_single_turn_converted.grid(row=1, column=1)
encoder_single_turn_converted_text = tk.StringVar()
ent_encoder_single_turn_converted = tk.Entry(master=fm_biss, textvariable=encoder_single_turn_converted_text, width=7); ent_encoder_single_turn_converted.grid(row=1, column=2)
encoder_single_turn_converted_text.set("0")

lbl_encoder_error = tk.Label(master=fm_biss, text="Error + Warning"); lbl_encoder_error.grid(row=2, column=1)
encoder_error_text = tk.StringVar()
ent_encoder_error = tk.Entry(master=fm_biss, textvariable=encoder_error_text, width=7); ent_encoder_error.grid(row=2, column=2)
encoder_error_text.set("0")

lbl_encoder_crc_calculated = tk.Label(master=fm_biss, text="CRC calculated"); lbl_encoder_crc_calculated.grid(row=3, column=1)
encoder_crc_calculated_text = tk.StringVar()
ent_encoder_crc_calculated = tk.Entry(master=fm_biss, textvariable=encoder_crc_calculated_text, width=7); ent_encoder_crc_calculated.grid(row=3, column=2)
encoder_crc_calculated_text.set("0")

lbl_encoder_crc_received = tk.Label(master=fm_biss, text="CRC received"); lbl_encoder_crc_received.grid(row=4, column=1)
encoder_crc_received_text = tk.StringVar()
ent_encoder_crc_received = tk.Entry(master=fm_biss, textvariable=encoder_crc_received_text, width=7); ent_encoder_crc_received.grid(row=4, column=2)
encoder_crc_received_text.set("0")

lbl_encoder_clock_edges = tk.Label(master=fm_biss, text="Number of clock edges"); lbl_encoder_clock_edges.grid(row=5, column=1)
encoder_clock_edges_text = tk.StringVar()
ent_encoder_clock_edges = tk.Entry(master=fm_biss, textvariable=encoder_clock_edges_text, width=7); ent_encoder_clock_edges.grid(row=5, column=2)
encoder_clock_edges_text.set("0")
...
fm_biss.grid(sticky="W", row=2, column=0)
...
def read_every_second():
    ...
    if reply == 1: # reply in SPI transmit buffer
        ...
        single_turn = 256*value[2] + value[3]
        encoder_single_turn_converted_text.set(single_turn)
        # get Error + Warning + CRC
        value = SpiWriteCommand([0x00, 0x00, 0x00, 0x00])
        #print(f"ST (binary): {value[0]:02x} {value[1]:02x} {value[2]:02x} {value[3]:02x}")
        # use single turn 13bit value
        absolute_degree = single_turn / 8192 * 360
